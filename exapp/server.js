...
/*	PUT: server/db/questions/new with body "name=<your_name>&
...
app.put('/db/new', (req, res) => {
	...
	var title = req.body.title; var invite = tools.generateKey(); var date = new Date(); 
	var type = req.body.type; var correct = req.body.correct;
	var A = req.body.a;	var B = req.body.b;	var C = req.body.c;	var D = req.body.d;
	con.then(() => {
		return db.query("INSERT INTO `surveys` (`creator`, `invite`, `title`, `date`, `type`, `correct`, `answer_A`, `answer_B`, `answer_C`, `answer_D`) VALUES (?,?,?,?,?,?,?,?,?,?);", [creator,invite,title,date,type,correct,A,B,C,D]);
	}).then((result) => {
		res.status(201).send();
	}).catch((err) => {
		console.log(err);
